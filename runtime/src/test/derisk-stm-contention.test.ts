...
import type { ContextPath, World } from "../ast/types.ts";
import {
  makeStoreRefsSharing,
  storeFromRefs,
} from "../effect/context-store.ts";
import { getAt } from "../effect/path.ts";
    ...
    () =>
      Effect.gen(function* () {
        ...
        const pipsEvening = Effect.gen(function* () {
          yield* pip.beginTurn;
          yield* pip.setStaged(diary("pips_line"), "I planted something good today.");
        ...
        const rainsNight = Effect.gen(function* () {
          ...
          yield* rain.setStaged(diary("rains_line"), "It rained all night, softly.");
