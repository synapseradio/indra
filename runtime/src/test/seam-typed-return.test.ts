...
import { Inference, InferenceStub } from "../baml/inference.ts";
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(getAt(world, { ns: "context", segments: ["reasoning"] })).toBeUndefined();
      }).pipe(Effect.provide(InferenceStub(stub))),
  );

  it.effect("the stub returns a structurally typed result, not parsed text", () =>
    Effect.gen(function* () {
      const inference = yield* Inference;
