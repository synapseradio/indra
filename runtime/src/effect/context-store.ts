...
import type { ContextPath, Json, World } from "../ast/types.ts";
import { getAt, isReadOnly, pathToString, setAt } from "./path.ts";
import { ReadOnlyViolationError } from "./errors.ts";
...
export type Patch = ReadonlyMap<string, { readonly path: ContextPath; readonly value: Json }>;
...
export interface ContextStore {
  ...
  readonly setPrivileged: (path: ContextPath, value: Json) => Effect.Effect<void>;
...
export const storeFromRefs = (refs: StoreRefs): ContextStore => ({
  ...
  setImmediate: (path, value) =>
    Effect.zipRight(
      guardWritable(path),
      STM.commit(TRef.update(refs.sequenceOverlay, (p) => patchSet(p, path, value))),
    ),

  setStaged: (path, value) =>
    Effect.zipRight(
      guardWritable(path),
      STM.commit(TRef.update(refs.stagedOverlay, (p) => patchSet(p, path, value))),
    ),
