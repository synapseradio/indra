...
import { ToolInvocationError as ToolInvocationErrorClass } from "@indra/runtime-contracts";
import { Effect } from "effect";
import { Inference } from "~/baml/inference";
import { ContextStoreTag } from "./context-store.layer";
...
const evalGuard = (
  ...
  Effect.gen(function* () {
    switch (guard.kind) {
      case "is":
        return deepEqual(
          yield* evalValue(guard.left),
          yield* evalValue(guard.right),
        );
      case "isNot":
        return !deepEqual(
          yield* evalValue(guard.left),
