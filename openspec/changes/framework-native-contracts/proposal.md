...
- **Inference points become direct typed function references.** Each inference point holds the inference function it calls directly, not a string key bound through a registry at load. One function may be referenced by many points across many actors, so an actor's definition holds the function references its points use rather than a function of its own.
- **Personas stop being a first-class construct.** The persona data record (`{identity, rules, understands}`) and its `as:` / `become: @persona` adoption are removed. An inference point's system constraints live inside the typed inference function it references — its BAML definition or its typed input — not in a separate record the runtime reads. Persona stays a mental-model term, never a framework construct.
- **"Inference point" and "inference leaf" collapse to one term.** The single marked site inside an actor where the model is consulted is the **inference point** throughout; "inference leaf" is retired as a synonym, and the separate "an actor with no sub-actors" sense becomes **atomic actor**.
